...
Sodium publishes its API package on a Maven repository that you can depend on in your build script. Use `modImplementation` for Fabric.
...
Entrypoint classes that Sodium calls to run your options registration code are declared in your mod's metadata file.
