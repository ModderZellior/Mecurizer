...
public class ChunkBuilder {
    ...
    private static int getOptimalThreadCount() {
        int processors = getMaxThreadCount();
        ...
        return Mth.clamp(processors / 6, 1, 3);
    }

    private static int getThreadCount() {
        int requested = SodiumClientMod.options().performance.chunkBuilderThreads;
        if (requested == 0 && MercurizerRuntimePolicy.preferSafeStagingPath()) {
            return 1;
        }

        if (requested == 0) {
            return MercurizerRuntimePolicy.getEffectiveChunkWorkerCount(getOptimalThreadCount());
        }

        return Math.min(requested, getMaxThreadCount());
    }
