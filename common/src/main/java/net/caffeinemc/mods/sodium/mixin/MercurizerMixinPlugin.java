...
public class MercurizerMixinPlugin implements IMixinConfigPlugin {
    private static final String MIXIN_PACKAGE_ROOT = "net.caffeinemc.mods.sodium.mixin.";

    private final Logger logger = LogManager.getLogger("Mercurizer");
    ...
    public void onLoad(String mixinPackage) {
        try {
            this.config = MixinConfig.load(new File("./config/mercurizer-mixins.properties"));
        } catch (Exception e) {
            throw new RuntimeException("Could not load configuration file for Mercurizer", e);
        }
        ...
        if (dependencyResolutionFailed) {
            this.logger.error("Not applying any Mercurizer mixins; dependency resolution has failed.");
        }

        this.logger.info("Loaded configuration file for Mercurizer: {} options available, {} override(s) found",
