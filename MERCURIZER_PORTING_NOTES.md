...
## Branch-Specific Note For 1.21.11

This branch is compared against `sodium-1.21.11-stable`, not against the `26.1.x` line.

That means:

- version wiring must follow `1.21.11`
- Fabric API module names and compatibility ranges must follow the `1.21.11` upstream baseline
- copied `26.1.x` assumptions should not be used as the source of truth for this branch
...
Compared with `sodium-1.21.11-stable`:

- `173` changed files
- `41` files only in Mercurizer
- `71` files only in upstream Sodium
...
The following Mercurizer-specific areas are present in this branch:

- `MercurizerRuntimePolicy.java`
- Mercurizer runtime-policy call sites in staging, chunk building, render upload throttling, CPU render-ahead, no-error context setup, and sort behavior
- `MercurizerMixinPlugin.java`
- `MercurizerResourcePackMetadata.java`
- Mercurizer Fabric entrypoints
- Mercurizer mixin/access widener/resource naming

This branch should be treated as a real Mercurizer port, not just a renamed Sodium branch.
